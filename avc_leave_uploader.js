(async()=>{
  ...
  c.innerHTML=`
    ...
    <button id="dl" style="background:#000;color:#fff;padding:10px 14px;border:none;border-radius:8px;margin:8px;">Download Template</button>
    <input type="file" id="up" accept=".xlsx" style="margin:8px;">
    <button id="ex" style="background:#217346;color:#fff;padding:10px 14px;border:none;border-radius:8px;margin:8px;">Export to Excel</button>

    <div style="margin-top:20px;">
      <button id="closeBtn" style="background:maroon;color:#fff;padding:7px 14px;border:none;border-radius:8px;">Close</button>
    </div>

    <div id="footerText" style="visibility:hidden;margin-top:20px;font-size:13px;">
  ...
  dl.onclick=()=>{
    ...
    let ws=XLSX.utils.aoa_to_sheet([
      ["Emp ID","Attendance Date","Leave Type","Remarks"]
    ]);
  ...
  up.onchange=async e=>{
    ...
    let rows=XLSX.utils.sheet_to_json(
      ...
      {header:1}
    ).slice(1).filter(r=>r[0]&&r[1]&&r[2]&&r[3]);
    ...
    (function next(){
      ...
      setTimeout(()=>{
        ...
        document.querySelectorAll("table tbody tr").forEach(r=>{
          let txt=r.innerText.replace(/\s+/g," ");
